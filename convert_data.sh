...
ENTITY_TYPES_IN=data/entity_types/entity_types.csv
ENTITY_TYPES_OUT=source/vdb_entity_type_definitions.cpp

# Input/output files for object types.
OBJECT_TYPES_IN=data/object_types/object_types.csv
OBJECT_TYPES_OUT=source/vdb_object_type_definitions.cpp

# Output files for master header/source.
ENUMS_H_OUT=source/vdb_enums.h
ENUMS_CPP_OUT=source/vdb_enums.cpp
...
if [ "$CONVERT" == true ] || [ "$FORCE_CONVERT" == true ]; then
    ...
    echo "#include \"vdb_entity_type.h\"" >> "$ENTITY_TYPES_OUT"
    echo "" >> "$ENTITY_TYPES_OUT"
    echo "void vdb::entity_type_data::add_all(void)" >> "$ENTITY_TYPES_OUT"
    echo "{" >> "$ENTITY_TYPES_OUT"
    while IFS='' read line;
    do
        if [ -z "$line" ]; then
...
if [ "$CONVERT" == true ] || [ "$FORCE_CONVERT" == true ]; then
    ...
    echo "#include \"vdb_object_type.h\"" >> "$OBJECT_TYPES_OUT"
    echo "" >> "$OBJECT_TYPES_OUT"
    echo "void vdb::object_type_data::add_all(void)" >> "$OBJECT_TYPES_OUT"
    echo "{" >> "$OBJECT_TYPES_OUT"
    while IFS='' read line;
    do
        if [ -z "$line" ]; then
...
CONVERTED=false
ENUM_NAMES=()
for csv in `ls data/enumerations`; do
    ENUM_IN="data/enumerations/$csv"
    ENUM_OUT=`echo "source/enumerations/$csv" | sed 's/.csv/.h/'`
...
if [ "$CONVERTED" == true ]; then
    ...
    echo "" >> "$ENUMS_H_OUT"
    echo "#ifndef VDB_ENUMS_H" >> "$ENUMS_H_OUT"
    echo "#define VDB_ENUMS_H" >> "$ENUMS_H_OUT"
    echo "" >> "$ENUMS_H_OUT"
    echo "#include <ostream>" >> "$ENUMS_H_OUT"
    echo "" >> "$ENUMS_H_OUT"
    echo "#include \"vdb_enumerations.h\"" >> "$ENUMS_H_OUT"
    echo "" >> "$ENUMS_H_OUT"
    ...
    for NAME in "${ENUM_NAMES[@]}"; do
        NAME_UPPER=`echo "$NAME" | awk '{print toupper($0)}'`
        echo "" >> "$ENUMS_H_OUT"
        echo "inline std::ostream &operator<<(std::ostream &stream, vdb::${NAME}_e value)" >> "$ENUMS_H_OUT"
        echo "{" >> "$ENUMS_H_OUT"
        echo "    return (stream << vdb::enumerations::get_name(ENUM_${NAME_UPPER}, value));" >> "$ENUMS_H_OUT"
        echo "}" >> "$ENUMS_H_OUT"
    done < $ENUM_IN
    echo "" >> "$ENUMS_H_OUT"
    ...
    echo "" >> "$ENUMS_CPP_OUT"
    echo "#include \"vdb_enumerations.h\"" >> "$ENUMS_CPP_OUT"
    echo "#include \"vdb_enums.h\"" >> "$ENUMS_CPP_OUT"
    echo "" >> "$ENUMS_CPP_OUT"
    echo "void vdb::enumerations::add_all(void)" >> "$ENUMS_CPP_OUT"
    echo "{" >> "$ENUMS_CPP_OUT"
    for csv in `ls data/enumerations`; do
        ENUM_IN="data/enumerations/$csv"
        ENUM_NAME_UPPER=`echo "$csv" | sed 's/.csv//' | awk '{print toupper($0)}'`
